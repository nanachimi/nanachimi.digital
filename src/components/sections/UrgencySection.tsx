...
import { TrendingUp, Clock, Zap, Rocket, ArrowRight } from "lucide-react";
...
const reasons = [
  ...
  },
  {
    ...
    description:
      "Ihre Konkurrenz baut bereits. Jeden Monat, den Sie warten, ist ein Monat, in dem jemand anderes Ihre Nische besetzt.",
  ...
  },
  {
    ...
    description:
      "Moderne Technologien ermöglichen es, in Tagen statt Monaten die erste Version zu starten. Die Einstiegshürde war noch nie so niedrig.",
    color: "text-blue-400",
...
export function UrgencySection() {
  ...
  const { config: abConfig, trackImpression } = useABTest("urgency-heading");
      ...
      <div className="container relative mx-auto px-4 md:px-6">
        ...
        <div className={`mx-auto max-w-3xl text-center mb-16 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          ...
          <h2 className="text-3xl font-black tracking-tight text-white md:text-4xl lg:text-5xl">
            {abConfig.heading ? (
              <span dangerouslySetInnerHTML={{ __html: abConfig.heading }} />
            ) : (
              <>
                Ihre Konkurrenz baut bereits.{" "}
          ...
          <p className="mt-6 text-lg text-[#8B8F97] max-w-2xl mx-auto">
            Jede Woche entstehen neue digitale Produkte. Wer jetzt handelt,
            sichert sich einen Vorsprung — wer wartet, verliert Marktanteile.
          </p>
        </div>
        ...
        <div className={`mx-auto max-w-3xl grid grid-cols-3 gap-4 mb-14 transition-all duration-700 delay-200 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <div className="text-center rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
            ...
            <div className="text-xs text-[#6a6e76] mt-1">Bis zum ersten MVP</div>
          </div>
          <div className="text-center rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
            <div className="text-3xl font-black text-emerald-400">24/7</div>
            <div className="text-xs text-[#6a6e76] mt-1">Ihre Lösung arbeitet non-stop</div>
          </div>
          <div className="text-center rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
            <div className="text-3xl font-black text-blue-400">
              <AnimatedCounter target={10} suffix="+" />
            </div>
            <div className="text-xs text-[#6a6e76] mt-1">Jahre Erfahrung</div>
          </div>
        </div>

        {/* Reason cards with staggered animation */}
        ...
        <div className={`mt-14 text-center transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} style={{ transitionDelay: visible ? "1000ms" : "0ms" }}>
          ...
          <p className="mt-4 text-sm text-[#6a6e76]">
            Kostenlose Einschätzung in 3 Minuten — kein Verkaufsgespräch
          </p>
        </div>
      </div>
    </section>
